...
bool ShouldDropIncomingBySessionFilter(const std::string &message,
  ...
  if (transport_name && transport_name[0] != '\0') {
    LOGW(transport_name << " drop incoming message for inactive session_id: "
                        << session_id);
  } else {
    LOGW("Drop incoming message for inactive session_id: " << session_id);
  }
  return true;
}
